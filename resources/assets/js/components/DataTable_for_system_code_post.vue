<template>
    <!-- Column selectors -->
    <div>
        <table class="table datatable-system-code-title">
            <thead>
            <tr>
                <th>NO.</th>
                <th>描 述</th>
            ...
            <tbody @click="showEditModel($event)">
                <tr v-for="tit in title">
                    <td>{{ tit.order?tit.order:'0' }}</td> <!-- 默认排序为order字段，datatable必须将order放到第一列才行 -->
                    <td>{{ tit.description }}</td><!-- 上级部门的id -->
                    <td>{{ tit.order?tit.order:'0' }}</td>
                    <td><span :class="[tit.isFirst==='T'? 'label label-success' : 'label label-default']">{{ tit.isFirst==='T'?'是':'否' }}</span></td>
                    <td><span :class="[tit.status==='T'? 'label label-success' : 'label label-danger']">{{ tit.status==='T'?'已启用':'未启用' }}</span></td>
                    <td>{{ tit.remarks?tit.remarks:'/' }}</td>
                    <td>{{ tit.created_at }}</td>
            ...
            <div slot="body">
                <div class="form-group">
                    <div class="row">
                        ...
                        <div class="col-sm-6">
                            <label>排序NO.</label>
                            <input type="text" v-model="newTitle.order" placeholder="数值越大排名越靠前,默认为0" class="form-control">
                        </div>
                    </div>
                </div>
                <div class="form-group">
                    <div class="row">
                        <div class="col-sm-6">
                            ...
                            <div>
                                <toggle-button @change="changeIsFirst" :sync="true" :v-model="newTitle.isFirst" :value="newTitle.isFirst==='T'" :width="58" :height="34" :labels="{checked: '是', unchecked: '否'}"/>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-6">
                            ...
                            <!--<input type="text" v-model="newDepartment.status" placeholder="" class="form-control">-->
                            <div>
                                <toggle-button @change="changeStatus" :sync="true" :v-model="newTitle.status" :value="newTitle.status==='T'" :width="140" :height="34" :labels="{checked: '当前处于启用状态', unchecked: '当前处于停用状态'}"/>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="form-group">
                    <div class="row">
                        <div class="col-sm-12">
                            <label>备注信息</label>
                            <input type="text" v-model="newTitle.remarks" placeholder="备注信息" class="form-control">
                        </div>
                    </div>
                </div>
            </div>
        ...
        <department_modal v-show="showEditGenderModel" @close="closeEditModal" @commit="addTitle">
...
<script>
    ...
    export default {
        ...
        methods: {
            getTitle() {
               return axios.get('/system_code/get/title')
            },
            showAddTitle() {
                //模态框重新显示之前，清空所有的验证提示消息。
                this.errors.clear();
                this.showAddTitleModel = true;
                //模态框弹出的时候禁止底层body滚动
                $('body').css('overflow','hidden');
                console.log('showAddTitle')
            },
            closeAddTitle() {
                this.showAddTitleModel = false;
                //模态框关闭的时候清空表中的数据为初始值
                this.newTitle = {
                    description:'',
                    order:null,
            ...
            showEditModel(e) {
                ...
                if(buttonClass.indexOf('edit') !== -1) {
                    ...
                    this.reloadOptions();
                    $('body').css('overflow','hidden');
                    let table = $('.datatable-system-code-title').DataTable();
...
//            },
            changeStatus() {
                if(this.newTitle.status==='T') {
                    this.newTitle.status = 'F'
                }else {
                    this.newTitle.status = 'T'
                }
            },
            changeIsFirst() {
                if(this.newTitle.isFirst==='T') {
                    this.newTitle.isFirst = 'F'
                }else {
                    this.newTitle.isFirst = 'T'
                }
            },
            checkDescription() {
            ...
            addTitle() {
                this.$validator.validateAll().then((result)=> {//验证是否符合表单规则
                    if(result) {//如果符合才提交
                        axios.post('/system_code/add/title',{title:this.newTitle}).then(res=> {
                            let table = $('.datatable-system-code-title').DataTable();
                            //增加一行数据
//                            table.row.add([
//                                this.newTitle.order,
//                                this.newTitle.description,
//                                this.newTitle.order?this.newTitle.order:'0',
//                                this.newTitle.isFirst==='T'?'<span class="label label-success">是</span>':'<span class="label label-default">否</span>',
//                                this.newTitle.status==='T'?'<span class="label label-success">已启用</span>':'<span class="label label-danger">未启用</span>',
//                                this.newTitle.remarks?this.newTitle.remarks:'/',
...
//                            ]).draw(false);
            ...
            tableSetup() {
                ...
                $(function () {
                    ...
                    // Column selectors
                    let t = $('.datatable-system-code-title').DataTable({
                        buttons: {
                            buttons: [
                                {
                                    extend: 'copyHtml5',
                    ...
                    //增加一个按钮在后面用来添加项目用的
                    $('#DataTables_Table_1_filter').after(
                        '<div class="dt-buttons">  ' +
                          '<button class="showAddTitle dt-button btn btn-primary" tabindex="0" type="button">' +
